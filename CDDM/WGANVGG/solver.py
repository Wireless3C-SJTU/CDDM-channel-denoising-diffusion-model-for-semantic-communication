...
class Solver(object):
    def __init__(self, args,config,CDDM_config):
        self.mode = args.mode
        self.load_mode = args.load_mode
        #self.data_loader = data_loader
        self.config=config
        self.CDDM_config=CDDM_config
        if args.device:
    ...
    def train(self):
        trainLoader, _ = get_loader(self.CDDM_config)
        train_losses = []
        total_iters = 0
        ...
        for epoch in range(1, self.num_epochs):
            with tqdm(trainLoader, dynamic_ncols=True) as tqdmDataLoader:
                for iter_, input in enumerate(tqdmDataLoader):
                    total_iters += 1
                    # add 1 channel
