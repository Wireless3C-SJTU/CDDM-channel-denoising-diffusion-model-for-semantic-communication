...
class WGAN_VGG_generator(nn.Module):
    def __init__(self,in_channels=36):
        ...
        layers = [nn.Conv2d(in_channels,32,3,1,1), nn.Tanh()]
        for i in range(2, 8):
            layers.append(nn.Conv2d(32,32,3,1,1))
            layers.append(nn.ReLU())
        layers.extend([nn.Conv2d(32,in_channels,3,1,1), nn.Tanh()])
...
class WGAN_VGG_discriminator(nn.Module):
    def __init__(self, input_size,in_channels=36):
        ...
        layers = []
        ch_stride_set = [(in_channels,64,1),(64,64,2),(64,128,1),(128,128,2),(128,256,1),(256,256,1)]
        ...
        self.output_size=input_size//(2**2)
    ...
    def forward(self, x):
        out = self.net(x)
        #print(out.shape)
        out = out.reshape(-1, 256*self.output_size*self.output_size)
...
class WGAN_VGG(nn.Module):
    ...
    def __init__(self, input_size=64,in_channels=36):
        super(WGAN_VGG, self).__init__()
        self.generator = WGAN_VGG_generator(in_channels=in_channels)
        self.discriminator = WGAN_VGG_discriminator(input_size,in_channels=in_channels)
    ...
    def g_loss(self, x, y, perceptual=True, return_p=False):
        ...
        if perceptual:
            p_loss = torch.nn.MSELoss()(fake, y)
            loss = g_loss + ( 1* p_loss)
            #print(g_loss,p_loss)
        else:
            p_loss = None
            loss = g_loss
